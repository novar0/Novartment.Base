	...
	public class EbmlParserTests
	{
		...
		public void EbmlParser_Read ()
		{
			using (var fs = new FileStream (@"test.mkv", FileMode.Open, FileAccess.Read))
			{
				...
				Assert.True (reader.MoveNextAsync (CancellationToken.None).Result);
				Assert.Equal (0x1a45dfa3UL, reader.Current.Id);
				var subReader1 = reader.Current.ReadSubElements ();
				Assert.True (subReader1.MoveNextAsync (CancellationToken.None).Result);
				...
				Assert.True (subReader2.MoveNextAsync (CancellationToken.None).Result);
				...
				Assert.True (subReader3.MoveNextAsync (CancellationToken.None).Result);
				...
				Assert.True (subReader4.MoveNextAsync (CancellationToken.None).Result);
