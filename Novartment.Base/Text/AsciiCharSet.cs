...
namespace Novartment.Base.Text
{
	...
	public static class AsciiCharSet
	{
		...
		#region static method IsAllOfClass
		...
		#region static method IsAnyOfClass
		...
		#region static method GetString
		...
		#region method IsValidInternetDomainName
		...
		public static bool IsValidInternetDomainName (string name)
		{
			...
			for (position = 0; position < name.Length; position++)
			{
				...
				if (c == '.')
				{
					if ((position - lastDotPosition) < 2) // от точки до точки должен быть мин. один символ, т.е. расстояние >= 2
					{
						return false;
					}
