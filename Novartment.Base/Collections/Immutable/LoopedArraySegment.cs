...
namespace Novartment.Base.Collections.Immutable
{
		...
		IStructuralEquatable
	{
		...
		{
			return new _ArraySegmentLoopedEnumerator (this);
		}
		...
		#region struct _ArraySegmentLoopedEnumerator

		internal struct _ArraySegmentLoopedEnumerator : IEnumerator<T>, IDisposable, IEnumerator
		{
			private readonly LoopedArraySegment<T> _data;
			private int _index;
			private T _currentElement;

			internal _ArraySegmentLoopedEnumerator (LoopedArraySegment<T> data)
			{
				_data = data;
				_index = -1;
