...
namespace Novartment.Base.Net.Smtp
{
		...
		IDisposable
	{
		...
		private ExpectedInputType _expectedInput = ExpectedInputType.Command;
		...
		internal SmtpDeliveryProtocolSession (
			ISmtpCommandReplyConnectionSenderReceiver sender,
		...
		internal async Task<bool> ReceiveCommandSendReplyAsync (CancellationToken cancellationToken)
		{
			...
			catch (UnacceptableSmtpMailboxException excpt) // 553  Requested action not taken: mailbox name not allowed
			{
				_logger?.Warn (String.Format ("Not acceptable mailbox {0}", excpt.Mailbox.ToAngleString ()));
				reply = SmtpReply.MailboxNotAllowed;
			}
			catch (BadSequenceOfSmtpCommandsException) // 503  Bad sequence of commands
			{
				reply = SmtpReply.BadSequenceOfCommands;
			}
			catch (NoValidRecipientsException) // 554 No valid recipients
			{
			...
			if (reply.Code == 334) // server challenge
			{
			}
		...
		{
			if (_authenticatedUser != null) // уже аутентифицировались, повтор невозможен
			{
			...
			if (!_sender.TlsEstablished) // TLS не запущен
			{
				// RFC 4954 part 4:
			...
			{
				_expectedInput = ExpectedInputType.AuthenticationResponse;
		...
		{
			_expectedInput = ExpectedInputType.Command;
		...
		{
			if ((!_clientIdentified) || // не поздоровались
				(_currentTransaction != null)) // транзакция уже начата
			{
				throw new BadSequenceOfSmtpCommandsException ();
			}
			if ((_securityParameters.ServerCertificate != null) &&
				!_sender.TlsEstablished) // требуется шифрование, а соединение не TLS
			{
				return Task.FromResult (SmtpReply.MustUseStartTlsFirst.AllowGrouping ());
			}
			if ((_securityParameters.ClientAuthenticator != null) &&
				(_authenticatedUser == null)) // требуется аутентификация, а клиент не аутентифицировался
			{
		...
		private async Task<SmtpReplyWithGroupingMark> ProcessCommandMailFromFinalizer (Task task,
		...
		{
			if ((!_clientIdentified) || // не поздоровались
				(_currentTransaction == null) || // транзакция не начата
				(_chunkingAgency != null)) // уже начат режим передачи (порциями)
			{
				throw new BadSequenceOfSmtpCommandsException ();
			}
		...
		private SmtpReplyWithGroupingMark ProcessCommandData ()
		{
			...
			if ((!_clientIdentified) || // не поздоровались
				(_currentTransaction == null) || // транзакция не начата
				(_chunkingAgency != null) || // уже начат другой режим передачи (порциями)
				(_currentTransactionRequestedEncoding == ContentTransferEncoding.Binary)) // режим BINARYMIME не может использоваться
			{
				throw new BadSequenceOfSmtpCommandsException ();
			}
			...
			_expectedInput = ExpectedInputType.Data;
		...
		{
			if (!_clientIdentified || (_currentTransaction == null)) // не поздоровались или не начали транзакцию
			{
			...
			if (_currentTransactionAcceptedRecipients <= 0) // не указаны получатели
			{
				ResetTransaction ();
				return bdatCommand.Source
					.SkipToEndAsync (cancellationToken) // пропускаем все данные (предотвратить их передачу невозможно)
			...
			if (_chunkingAgency == null) // порция - первая
			{
				if (bdatCommand.IsLast) // она же - последняя
				{
					// единственная порция
		...
		public void Dispose ()
		{
			...
			if (oldValue == 0)
			{
				...
				catch (IOException) { } // посылка прощального ответа необязательна, поэтому игнорируем исключения если связи уже нет
