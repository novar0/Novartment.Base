...
{
	public class HeaderValueParameterEncoderTests
	{
		[Fact]
		[Trait ("Category", "Mime")]
		public void Parse ()
		{
			var segments = HeaderValueParameterEncoder.Parse ("charset", "koi8-r");
			Assert.Equal ("charset=koi8-r", segments.GetNextSegment ());
			Assert.Null (segments.GetNextSegment ());

			segments = HeaderValueParameterEncoder.Parse ("charset", "koi8 r");
			Assert.Equal ("charset=\"koi8 r\"", segments.GetNextSegment ());
			Assert.Null (segments.GetNextSegment ());

			segments = HeaderValueParameterEncoder.Parse ("charset", "функции");
			Assert.Equal ("charset*0*=utf-8''%D1%84%D1%83%D0%BD%D0%BA%D1%86%D0%B8%D0%B8", segments.GetNextSegment ());
			Assert.Null (segments.GetNextSegment ());

			segments = HeaderValueParameterEncoder.Parse ("filename", "This document specifies an Internet standards track protocol for the функции and requests discussion and suggestions.txt");
