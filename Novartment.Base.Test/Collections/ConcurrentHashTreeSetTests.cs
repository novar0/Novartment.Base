...
namespace Novartment.Base.Test
{
	public class ConcurrentHashTreeSetTests
	{
		...
		{
			var set = new ConcurrentHashTreeSet<MockStr> ();
			var offset = 99;
			var vvv = Range (-offset, offset * 2).Select (n => new MockStr (n, 0.3d)).ToArray ();
		...
		public void BalanceAndOrder ()
		{
			...
			var vvv = Range (-offset, offset * 2).Select (n => new MockStr (n, 0.0d)).ToArray ();
			set.Add (vvv[-20 + offset]);
			...
			set = new ConcurrentHashTreeSet<MockStr> ();
			vvv = Range (-offset, offset * 2).Select (n => new MockStr (n, 1.0d)).ToArray ();
			set.Add (vvv[-20 + offset]);
			set.Add (vvv[105 + offset]);
			set.Add (vvv[101 + offset]);
			set.Add (vvv[110 + offset]);
		...
		public class MockStr : IEquatable<MockStr>
		{
			private double _hashMod;

			public MockStr (int value, double hashMod)
			{
				this.Value = value;
				_hashMod = hashMod;
			}
			...
			public override string ToString () => string.Format ("{0}, {1}", Value, GetHashCode ());
